...
export const CategoryDropDown = ({ category, isActive, isNavigationHovered }: CategoryDDProps) => {
    ...
    return (
        <div className='relative'
            ...
            <div className='relative'>
                <Button variant="elevated" className={cn("h-11 px-4 bg-transparent border-primary rounded-full hover:bg-green-600 hover:border-primary text-black hover:text-white",
                    isActive && !isNavigationHovered && "bg-white text-black border-primary border-[2px]",
                ...
                {isOpen && category.subcategories && category.subcategories.length != 0 && (
                    <div className={cn("opacity-50 absolute -bottom-3 w-0 h-0 border-l-[10px] border-r-[10px] border-b-[10px]  border-l-transparent border-r-transparent border-b-black left-1/2 -translate-x-1.5", isOpen && "opacity-90 bg-transparent-50 ")} />
                )}
            </div>
            <SubCategoryMenu
                category={category}
